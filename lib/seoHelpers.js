 ...
 */
function mergeProductSEO(product, category = null, baseUrl = '') {
    const defaultCanonical = baseUrl && product.handle ? `${baseUrl}/products/${product.handle}` : null;
    ...
    if (!category) {
        return {
            ...
            canonical_url: product.canonical_url || defaultCanonical,
            robots: product.robots || 'index,follow',
            structured_data: product.structured_data || generateProductSchema(product, null, baseUrl)
        };
    }

    const categoryCanonical = baseUrl && category.slug ? `${baseUrl}/categories/${category.slug}` : null;
    ...
    return {
        ...
        structured_data: product.structured_data || generateProductSchema(product, category, baseUrl)
    };
}
 ...
 */
function generateProductSchema(product, category = null, baseUrl = '') {
    const schema = {
    ...
    if (product.price) {
        schema.offers = {
            ...
            "availability": (product.inventory_quantity > 0 || product.track_inventory === false)
                ? "https://schema.org/InStock"
                : "https://schema.org/OutOfStock"
        };
    }

    // Add Aggregate Rating if available (and valid)
    const hasRatings = product.rating_summary && 
                       (parseFloat(product.rating_summary.average_rating) > 0 || 
                        parseInt(product.rating_summary.total_ratings) > 0);

    if (hasRatings) {
        schema.aggregateRating = {
            "@type": "AggregateRating",
            "ratingValue": product.rating_summary.average_rating || 0,
            "reviewCount": product.rating_summary.total_ratings || 0,
            "bestRating": "5",
            "worstRating": "1"
        };
        ...
        schema.review = {
            ...
            "reviewRating": {
                "@type": "Rating",
                "ratingValue": product.rating_summary.average_rating || 5,
                "bestRating": "5"
            },
            "author": {
                "@type": "Person",
    ...
    // Add Brand/Vendor with Collection URL
    const vendorName = product.attributes?.vendor || product.vendor;
    if (vendorName) {
        schema.brand = {
            "@type": "Brand",
            "name": vendorName
        ...
        // Use the official collection slug fetched by the ProductService
        if (product.store_collection?.slug && baseUrl) {
            schema.brand.url = `${baseUrl}/collections/${product.store_collection.slug}`;
        }
    }
 ...
 */
function mergeCategorySEO(category, baseUrl = '') {
    return {
        title: category.name,
        ...
        canonical_url: category.canonical_url || (baseUrl && category.slug ? `${baseUrl}/categories/${category.slug}` : null),
 ...
 */
function generateBrandedSEO(category, attribute, clause, baseUrl = '', imageUrl = null) {
    const prefix = clause.prefix || '';
    const suffix = clause.suffix || '';
    const title = `${prefix}${category.name}${suffix}`.trim();

    // Description logic: seo_template → category.description → auto-generated
    ...
    } else if (category.description || category.meta_description) {
    ...
    // Image priority: cached clause image → category image → null
    const ogImage = imageUrl || category.image_url || category.og_image || null;
